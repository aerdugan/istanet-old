var KallyasConfig={searchDomain:'https://kallyas-template.net',enableChaserMenu:0,chaserMenuOffsetDisplay:150,enableStickyHeader:1,enableScrollSpy:0,enableBackToTop:1};(function($,window,document){"use strict";var $window=$(window),$html=$("html"),$body=$("body"),$header=$("#header");$(document).ready(function(){if(window.location.href.indexOf("file:///")>-1){alert("Local environment warning!! \n\nSome of the features of this template might not work properly if the .html file is opened locally. \n\nFor best results, please preview on a web server (either http://yourdomain.com/kallyas/ or from your computer with an emulated server eg: http://localhost/kallyas/ ). Please read the documentation to learn more. ")}});if(/^((?!chrome|android).)*safari/i.test(navigator.userAgent)){document.getElementsByTagName('body')[0].className+=' is-safari'}
	...
	var searchForms=$('.elm-searchbox__form');if(searchForms){searchForms.on('submit',function(e){var searchInput=$(this).find('.elm-searchbox__input'),__search=searchInput.val(),qs='site:'+KallyasConfig.searchDomain+' '+__search;searchInput.val(qs);window.setTimeout(function(){searchInput.val(__search)},1000);return!0})}(function(){var pathname=window.location.pathname,page=pathname.split(/[/ ]+/).pop(),menuItems=$('#menu-main-menu a');menuItems.each(function(){var mi=$(this),miHrefs=mi.attr("href"),miParents=mi.parents('li');if(page==miHrefs){miParents.addClass("active").siblings().removeClass('active')}})})();if(window.matchMedia('(min-width: 992px)').matches){if(KallyasConfig.enableChaserMenu&&!KallyasConfig.enableStickyHeader){var KT_Chaser={visible:!1,update:function($header,headerStyleNumber){if(headerStyleNumber<1){console.warn('[KT_Chaser::update] Error: Invalid header style number provided.');return}
		...
		$(window).scroll(function(e){var scrolled=$(window).scrollTop();if(scrolled>=forch){_chaser.addClass('visible');KT_Chaser.visible=!0}else if(scrolled<forch){_chaser.removeClass('visible');KT_Chaser.visible=!1}else{if(_chaser.hasClass('visible')&&$('#chaserMenuRow').html().trim()==''){KT_Chaser.visible=!0}}})}}}
	if(KallyasConfig.enableStickyHeader&&!KallyasConfig.enableChaserMenu){var header=$('#header');$(window).scroll(function(){var scrolled=$(window).scrollTop();if(scrolled<1){$body.removeClass('sticky-header')}else if(scrolled>0&&scrolled<=9){$body.addClass('sticky-header');header.removeClass('header--is-sticked').addClass('header--no-sticked')}else if(scrolled>=10){header.addClass('header--is-sticked')}else{header.removeClass('header--is-sticked').addClass('header--no-sticked')}})}
	...
	var selector=$("#current-date");if(selector&&selector.length>0){$.ajax({url:"php_helpers/date.php",success:function(data){$("#current-date").html(data)}})}
	...
	jQuery('#textplay .textplay-line').each(function(index,el){var words=jQuery(el).children('.textplay-word'),randlast;setInterval(function(){var rand=Math.floor((Math.random()*words.length));if(rand==randlast)
		rand=Math.floor((Math.random()*words.length));words.removeClass('active');setTimeout(function(){words.eq(rand).addClass('active')},300);randlast=rand},3000)});if(matchMedia('only screen and (min-width: 767px)').matches&&$html.hasClass('js')){var scrollSvg=$('.kl-slideshow, #page_header');if(scrollSvg&&scrollSvg.length>0){var scrollSvgHeight=(parseFloat(scrollSvg.css('height'))+20),svgEffect=scrollSvg.find('.screffect');$window.scroll(function(){var scrollPos=$window.scrollTop();if(scrollPos<scrollSvgHeight){var maxSkew=2;var effectPos=(maxSkew/scrollSvgHeight)*scrollPos;svgEffect.css({'-webkit-transform':'skewY(-'+effectPos+'deg)','transform':'skewY(-'+effectPos+'deg)'})}})}}
