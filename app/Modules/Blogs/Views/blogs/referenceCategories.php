...
<?= $this->section('breadcrumb') ?>
    <div class="app-toolbar-wrapper d-flex flex-stack flex-wrap gap-4 w-100">
        <div class="page-title d-flex flex-column justify-content-center gap-1 me-3">
            <h1 class="page-heading d-flex flex-column justify-content-center text-gray-900 fw-bold fs-3 m-0"><?= lang("App.menu_blog_category")?></h1>
            <ul class="breadcrumb breadcrumb-separatorless fw-semibold fs-7 my-0">
                ...
                <li class="breadcrumb-item text-muted"><?= lang("App.menu_blog_category")?></li>
            </ul>
        </div>
        <div class="d-flex align-items-center gap-2 gap-lg-3">
            <a href="#" class="btn btn-sm btn-primary"  data-bs-toggle="modal" data-bs-target="#addModal"><?= lang("App.blog_category_add")?></a>
        </div>
    </div>
<?= $this->endSection() ?>
<?= $this->section('content') ?>
        ...
        <div class="modal-dialog modal-dialog-centered mw-750px">
            <div class="modal-content">
                <div class="modal-header">
                    <h2><?= lang("App.blog_category_add")?></h2>
                    <div class="btn btn-sm btn-icon btn-active-color-primary" data-bs-dismiss="modal">
                        <i class="ki-outline ki-cross fs-1"></i>
                    </div>
                </div>
                <div class="modal-body scroll-y mx-5 mx-xl-2 my-7">
                    <form action="/blogs/categoryAdd" class="form" method="post">
                        <?= csrf_field() ?>
                        <div class="form-row">
                            <div class="mb-5">
    ...
    <div class="modal fade" id="editModal" tabindex="-1" role="dialog" data-bs-backdrop="static" data-bs-keyboard="false"  aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered mw-750px">
            <div class="modal-content">
                <div class="modal-header">
                    <h2><?= lang("App.blog_category_update")?></h2>
                    <div class="btn btn-sm btn-icon btn-active-color-primary" data-bs-dismiss="modal">
                        <i class="ki-outline ki-cross fs-1"></i>
                    </div>
                </div>
                <div class="modal-body scroll-y mx-5 mx-xl-2 my-7">
                    <form action="/blogs/categoryEdit" class="form" method="post">
                        <?= csrf_field() ?>
                        <div class="form-row">
                            <div class="mb-5">
                                <label class="form-label"><?= lang("App.title")?></label>
        ...
        $(document).ready(function () {
            ...
            $('#sortable-list').sortable({
                ...
                update: function () {
                    ...
                    $.ajax({
                        url: '<?= site_url("blogs/categoryRankUpdate") ?>',
                        type: 'POST',
                        dataType: 'json',
                        data: {
                            order: newOrder,
            ...
            $(document).on('change', '.toggle-isActive', function () {
                ...
                $.ajax({
                    url: '<?= site_url("blogs/categoryIsActiveSetter") ?>',
                    type: 'POST',
                    dataType: 'json',
                    data: {
                        id: id,
            ...
            $(document).on('click', '.btn-delete', function (e) {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            url: '<?= site_url("blogs/categoryDelete") ?>',
                            type: 'POST',
                            dataType: 'json',
                            data: {
                                id: id,
            ...
            $(document).on('submit', 'form[action="/blogs/categoryEdit"]', function (e) {
                const currentData = {
                    id: $('.id').val(),
