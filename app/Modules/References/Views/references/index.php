...
<?= $this->section('breadCrumbs') ?>
    <div class="app-toolbar-wrapper d-flex flex-stack flex-wrap gap-4 w-100">
        ...
        <div class="d-flex align-items-center gap-2 gap-lg-3">
            <a href="<?= base_url('admin/references/newForm')?>" class="btn btn-flex btn-primary h-40px fs-7 fw-bold <?= $btn_add['class']??''?>">
...
<?= $this->section('pageScripts') ?>
    ...
    <script>
        ...
        var KTUserList = function () {
            ...
            var initDatatable = function () {
                ...
                datatable = $(table).DataTable({
                    ...
                    ajax: {
                        url: "<?= base_url('admin/references/getReferences') ?>",
                        type: "POST",
                        data: function (d) {
                            d[csrfName] = csrfHash;
                            return d;
                ...
                $(document).on('click', '.delete-reference', function () {
                    ...
                    Swal.fire({
                        ...
                        if (result.isConfirmed) {
                            $.ajax({
                                url: "<?= base_url('admin/references/delete') ?>",
                                type: "POST",
            ...
            var handleToggles = function () {
                $(document).on('change', '.toggle-active, .toggle-cover', function () {
                    ...
                    $.ajax({
                        url: "<?= base_url('admin/references/toggle') ?>",
                        type: "POST",
                        data: {
                            id: id,
                            field: field,
        ...
        document.addEventListener("DOMContentLoaded", function () {
            $(document).on('click', '.generate-reference-btn', function (e) {
                e.preventDefault();
                const refId = $(this).data('id');
                const lang = $(this).data('lang');

                Swal.fire({
                    title: `${lang.toUpperCase()} referans oluşturulacak`,
                    showConfirmButton: false,
                ...
                fetch(`${baseUrl}/admin/references/generateLangVersion/${refId}/${lang}`)
                    .then(res => res.json())
                    .then(res => {
                        Swal.fire({
        ...
        $('#kt_user_table').on('row-reorder', function (e, diff, edit) {
            let order = [];

            for (let i = 0; i < diff.length; i++) {
                let rowId = diff[i].node.id.replace('row_', '');
                order.push(rowId);
            }

            if (order.length > 0) {
                $.ajax({
                    url: "<?= site_url('admin/references/updateRank') ?>",
                    type: 'POST',
                    data: {
                        order: order,
