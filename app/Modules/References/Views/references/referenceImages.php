...
<?= $this->section('pageScripts') ?>
    ...
    <script>
        let dropzone, uploadMode = "original", formEl, csrfName = "", csrfValue = "";

        window.addEventListener("DOMContentLoaded", () => {
            formEl = document.getElementById("kt_ecommerce_add_product_form");
            const uploadBaseUrl = formEl.getAttribute("action");
            const refreshUrl = formEl.getAttribute("data-url");

            // CSRF tokenları meta'dan al
            ...
            const getCsrf = () => ({ name: csrfName, value: csrfValue });

            const updateCsrf = (newToken) => {
                csrfName = newToken.token;
                csrfValue = newToken.hash;
                document.querySelector('meta[name="csrf_token_name"]').content = csrfName;
                document.querySelector('meta[name="csrf_token_value"]').content = csrfValue;
            };
            ...
            // Yükleme butonları
            ["original", "webp", "webp_resize"].forEach(mode => {
                document.getElementById(`btn-upload-${mode}`)?.addEventListener("click", () => {
                    uploadMode = mode;
            ...
            function attachAllEvents() {
                    ...
                    input.addEventListener("change", async function () {
                        ...
                        const res = await fetch(url, {
                            ...
                            headers: {
                                ...
                                "X-Requested-With": "XMLHttpRequest" // <<< BUNU EKLE
                            },
                            body: formData
                        });
                ...
                document.querySelectorAll('.isActive').forEach(input => {
                    input.addEventListener("change", async function () {
                        ...
                        const res = await fetch(url, {
                            ...
                            headers: {
                                "Content-Type": "application/x-www-form-urlencoded",
                                "X-Requested-With": "XMLHttpRequest" // ✅ Bu satır şart!
                            },
                            body: formData
                        });
                        ...
                        if (data.csrf) updateCsrf(data.csrf);
                        if (data.status === "success") {
                            Swal.fire({ icon: 'success', title: 'Durum Değiştirildi!', timer: 1000, showConfirmButton: false });
                            refreshImageList();
                        }
                    });
                });
                ...
                if (deleteAllBtn) {
                    deleteAllBtn.addEventListener("click", async () => {
                        ...
                        const res = await fetch(formEl.getAttribute("data-url").replace("refresh_image_list", "deleteAllImages"), {
                            method: "POST",
                            headers: { "Content-Type": "application/x-www-form-urlencoded" },
                            body: new URLSearchParams({ [csrf.name]: csrf.value })
                        });

                        const data = await res.json();
                        if (data.csrf) updateCsrf(data.csrf);
                        if (data.status === "success") {
                            Swal.fire({ icon: 'success', title: 'Tüm Resimler Silindi!', timer: 1000, showConfirmButton: false });
                    ...
                    update: function (event, ui) {
                        ...
                        $.ajax({
                            ...
                            success: function (res) {
                                if (res.csrf) updateCsrf(res.csrf);
